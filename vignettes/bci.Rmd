...
```{r}
number_of_species <- 5
bci <- get_bci(prevalent = number_of_species)
...
```{r}
covariates <- list(elevation = spatstat.data::bei.extra$elev, gradient = spatstat.data::bei.extra$grad)
```
...
```{r}
radii <- matrix(3, number_of_species, number_of_species)
```

Fitting the model to the dataset is then quite easy.

```{r}
fit <- ppjsdm::gibbsm(configuration, window = window, covariates = covariates, model = "Geyer", radius = radii)
summary(fit)
```