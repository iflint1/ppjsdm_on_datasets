...
This vignette explains how to use the `ppjsdm` package with the `seedlings2` dataset from `ecespa`.
...
```{r}
configuration <- Configuration(seedlings2$x, seedlings2$y, marks = seedlings2$marks)
window <- Rectangle_window(c(275, 600), c(375, 675))
```
...
```{r}
configuration <- Configuration(seedlings2$x, seedlings2$y)
```
